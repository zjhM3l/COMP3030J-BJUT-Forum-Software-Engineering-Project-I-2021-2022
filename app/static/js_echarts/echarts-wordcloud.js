    ...
    for (var j = 0; j < line.tokens.length; j++) {
      ...
      if (typeof tokenWidth === 'string' && tokenWidth.charAt(tokenWidth.length - 1) === '%') {
        ...
        if (tokenWidthNotSpecified) {
          ...
          // See `graphic/helper/text.static`.
          // (2) If image loaded, and `textBackgroundColor.image` is image src string,
          // use `imageHelper.findExistImage` to find cached image.
          // `imageHelper.findExistImage` will always be called here before
          // `imageHelper.createOrUpdateImage` in `graphic/helper/text.static#renderRichText`
...
PathProxy.prototype = {
  ...
  /**
   * If needs static implemented dashed line
   * @return {boolean}
   * @private
   */
  _needsDash: function () {
...
function linearMap(val, domain, range, clamp) {
  ...
  // See echarts/test/ut/spec/util/number.static#linearMap#accuracyError
...
var _default = env; // Zepto.static
// (c) 2010-2013 Thomas Fuchs
// Zepto.static may be freely distributed under the MIT license.
...
function onBgImageLoaded(image, textBackgroundColor) {
  // Replace image, so that `contain/text.static#parseRichText`
...
function setHoverStyle(el, hoverStyle, opt) {
  ...
  }) : setElementHoverStl(el, hoverStyle); // Duplicated function will be auto-ignored, see Eventful.static.
